-- Remove is_active column from features table
...
CREATE TABLE features_backup AS SELECT id, server_hostname, name, version, vendor_daemon,
    total_licenses, used_licenses, expiration_date, last_updated FROM features;

-- Recreate features table without is_active column
DROP TABLE features;
